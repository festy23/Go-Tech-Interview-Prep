...
export const questionsSql: Question[] = [
  ...
  },
  {
    ...
    correct: 1,
    explanation:
      "Современные СУБД (PostgreSQL, MySQL) имеют query optimizer, который переставляет таблицы в оптимальном порядке. Порядок в SQL-запросе — для читаемости. Но optimizer может ошибиться на сложных запросах — тогда помогают hints.",
  },
  {
    id: 6,
  ...
  },
  {
    ...
    correct: 1,
    explanation:
      "CTE: 1) Читаемость — именованный блок вместо вложенного подзапроса. 2) Переиспользование — можно ссылаться несколько раз. 3) Рекурсия — WITH RECURSIVE для деревьев. В PostgreSQL CTE материализуется (кэширует результат), что может быть и плюсом, и минусом.",
  },
  {
    id: 14,
  ...
  // ── Транзакции и блокировки — продвинутое (Q29-Q33) ───────────────────────
  {
    ...
    options: [
      ...
      "Транзакция видит снимок данных на момент своего начала — нет dirty read и non-repeatable read, но возможен serialization error",
  ...
  },
  {
    ...
    correct: 1,
    explanation:
      "В Go string не может быть nil (zero value — пустая строка), int64 не может быть nil (zero value — 0). Но SQL-столбец может быть NULL. sql.NullString имеет поле Valid (bool) для отличия NULL от пустой строки. Альтернатива: использовать *string.",
  },
  {
    id: 44,
  ...
  // ── Пул соединений (Q47-Q49) ─────────────────────────────────────────────
  {
    ...
    correct: 1,
    explanation:
      "MaxOpenConns: максимум одновременных соединений (по умолчанию — без лимита!). MaxIdleConns: сколько соединений держать в пуле для переиспользования. ConnMaxLifetime: пересоздавать соединение через N минут (для балансировщиков, смены паролей).",
  },
  {
    id: 48,
