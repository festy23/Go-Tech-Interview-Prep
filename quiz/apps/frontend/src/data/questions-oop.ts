...
export const questionsOop: Question[] = [
  ...
  },
  {
    ...
    options: [
      ...
      "Оба метода доступны, но вызов c.Hello() требует уточнения c.A.Hello() или c.B.Hello()",
  ...
  },
  {
    ...
    options: [
      ...
      "Интерфейсы используют динамическую диспетчеризацию, generics — мономорфизацию или словарную диспетчеризацию на этапе компиляции",
      "Generics быстрее, но не поддерживают методы",
      "Интерфейсы работают только со структурами, generics — с любыми типами",
    ],
    correct: 1,
    explanation:
      "Интерфейсы — runtime-полиморфизм: тип определяется в runtime через itable. Generics — compile-time: компилятор генерирует специализированный код или использует dict-based подход. Generics дают type safety без runtime overhead.",
  },
  {
    id: 27,
  ...
  },
  {
    ...
    correct: 1,
    explanation:
      "Functional Options — паттерн, где конфигурация передаётся через variadic функции-опции. Преимущества: значения по умолчанию, опциональность, расширяемость без ломающих изменений API.",
  },
  {
    id: 37,
