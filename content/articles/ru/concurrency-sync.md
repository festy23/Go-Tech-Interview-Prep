...
В Go 1.26 выражение `new(val)` создаёт указатель на значение напрямую — без промежуточной переменной. Например, `new(0)` возвращает `*int`, указывающий на `0`; `new(false)` — `*bool`. Это удобно в полях `sync.Pool.New` и других местах, где нужен указатель на примитив:

```go
// Go 1.26: new(val) — указатель на значение без промежуточной переменной
limit := new(100) // *int → 100, вместо: n := 100; &n
```
...
Go 1.26 расширил встроенную функцию `new`: теперь она принимает не только тип, но и значение, создавая указатель напрямую:
...
// Go 1.26
p := new(42) // *int → 42, одна строка

// Полезно для указателей на bool, int и другие примитивы
enabled := new(true)    // *bool
timeout := new(30)      // *int
```

Это устраняет необходимость в промежуточной переменной при инициализации полей структур, передаче аргументов или работе с `atomic.Pointer`:

```go
// Инициализация atomic.Pointer без промежуточной переменной
...
| `new(val)` (Go 1.26) | Указатель на значение без промежуточной переменной |
